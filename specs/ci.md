...
4. Assert a **successful capture pipeline**: host receives a valid frame (header + checksum + length), writes PNG (and optionally a short GIF), and image properties match the fixture (dimensions at minimum; golden or structural checks preferred over “file exists”). A zero-byte or truncated payload must fail the job.
